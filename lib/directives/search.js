    ...
    var Directive = function (Restangular) {
        ...
        directive.template = function (element, attr) {
            var html = '',
                ...
                buttonLabel = attr.label || 'Add Item',
            ...
            html += '<button class="add-new">' + buttonLabel + '</button>';
        ...
        directive.link = function ($scope, element, attr, lkEdit) {
            var $addNewElm = element.find('.add-new'),
                ...
                fields = attr.match || 'title',
                filter = (attr.filter && JSON.parse(attr.filter)) || {},
            ...
            if (attr.name) {
                varName += 'data.' + attr.name.replace(':', '.');
            } else {
                varName += 'model';
            }
            ...
            function runQuery(query) {
                ...
                if (query === '') {
                    // update with null to clear the list
                    return updateSearchItems(null);
                }
            ...
            $resultsElm.click(function (e) {
                ...
                if (!id || !e.target.classList.contains('add')) {
                    return;
                }
