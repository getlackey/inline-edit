...
module.exports.template = function (element, attr) {
    ...
    html += '<textarea class="input" data-ng-name="' + name + '" data-ng-model="' + name + '"';
    if (maxlength) {
...
module.exports.link = function ($scope, element, attr) {
    ...
    function resize() {
        setTimeout(function () {
            ...
            input[0].style.height = '2em';
            innerHeight = input[0].scrollHeight;
            input[0].style.height = (innerHeight && innerHeight + "px") || '2em';
        });
    }

    input.on('focus', resize);
